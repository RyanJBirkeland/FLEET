...
| `useSingleTaskReviewActions.ts` | Single-task review actions: shipIt, mergeLocally, createPr, requestRevision, rebase, discard. Surfaces conflict file list on merge failure, clickable PR URL via toast action, empty-queue toast, and fails loudly if revision feedback write fails. | `useSingleTaskReviewActions`, `UseSingleTaskReviewActionsResult` |
