...
| `BatchActionsToolbar.tsx` | code-review | Renders the batch action buttons (Merge All, Ship All, Create PRs, Discard All, Clear) with in-flight spinner state. Extracted from `TopBar` to eliminate JSX duplication. Count span has `aria-live="polite"`; each action button has an `aria-label` that includes the selection count. | `BatchActionsToolbar` |
