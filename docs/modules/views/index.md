...
| `PlannerView.tsx` | Multi-task workflow planning: epic management with task grouping, dependency management, and batch queuing. Opens `WorkbenchPanel` slide-over for task creation/editing. Tracks `assistantOpen` state (closes when epic deselected) for the upcoming `PlannerAssistant` drawer — passes `onOpenAssistant` down to `EpicDetail`. ErrorBoundary protected. | `PlannerView` (default) |
