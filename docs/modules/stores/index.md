...
|--------|---------|-------------|
| `sprintTasks.ts` | Primary sprint task store — CRUD, optimistic updates, polling merge. Pending update TTL is 5s; IPC errors clear the pending entry and revert via `loadData()`. Pending-update field names are typed `keyof SprintTask` (see `optimisticUpdateManager`) so typos fail at compile time. | `useSprintTasks`, `selectActiveTaskCount`, `selectReviewTaskCount`, `selectFailedTaskCount` |
