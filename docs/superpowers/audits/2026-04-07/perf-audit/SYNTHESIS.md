...
**Score: 9.0 = (Sev 3 × Conf 3) ÷ Effort 1** — *upgraded to Effort=S because the small reverse-order fix (F-t1-concur-6) is the immediate quick win; full batching is M and scores 4.5*
...
**Score: 9.0 = (Sev 3 × Conf 3) ÷ Effort 1** — *upgraded from Sev=Med because audit-trail tail queries are user-facing and tasks already hit 2,600+ rows*
**Canonical:** `F-t3-db-7`
...
*Override note: severity left at the lens-assigned Medium → score recomputed as `(2×3)/1 = 6.0`. See #5.*
...
*Top 10 re-sorted by score (note #4 collision is intentional — `F-t1-sysprof-1` and `F-t3-db-7` both compute to 9.0; see Score Table for full ordering).*
