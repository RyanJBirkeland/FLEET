...
`ConsoleHeader` is 32px tall and the task title — *the most important text on the screen* — renders at 12px. The meta strip (duration · phase · cost · ctx) uses 10px text. Status dot is a flat 8px circle with no glow. The header reads as a thin strip, not a header. Action buttons feel cramped.
