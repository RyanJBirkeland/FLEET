...
**Phase-0 cleanup rule:** If a soft-gate answer collapses a finding's severity to Low _and_ the finding is now negative-ROI, move it to the deferred list with the new reason. Don't silently drop it.
...
| `F-t4-ctx-2` / `F-t4-prompt-4` / `F-t4-ctx-9` | Build lazy-inject infrastructure + lazy-load skills (front-load index, defer details). The `ctx-9` "no lazy-inject mechanism exists" finding is the architectural enabler — implementing skill lazy-load _is_ the mechanism. | `src/main/agent-manager/prompt-composer.ts:265-268`, `src/main/agent-system/skills/index.ts` |
