...
- **Views**: 10 views in `src/renderer/src/views/` — Dashboard (⌘1), Agents (⌘2), IDE (⌘3, default), Task Pipeline (⌘4), PR Station (⌘5), Source Control (⌘6), Memory (⌘7), Cost (⌘8), Settings (⌘9), Task Workbench. Task Pipeline = execution monitoring (vertical pipeline flow); Task Workbench = planning/creation (form + AI copilot). View type union and `VIEW_LABELS` live in `panelLayout.ts`. Keyboard shortcuts mapped in `App.tsx` via `VIEW_SHORTCUT_MAP`.
...
- **Worktree file disappearance**: macOS worktrees frequently lose tracked files between operations (npm install, vitest runs). Always run `git checkout -- .` before working in a worktree if files appear missing. For long editing sessions, consider working directly on the branch in the main repo instead.
...
- **Agent prompt augmentation**: `run-agent.ts` auto-appends `## Setup` (npm install reminder) and `## Git Branch` (explicit branch name + "do not push to main") to every agent prompt. When adding new prompt sections, append after these. When testing prompt content in tests, use `expect.stringContaining()` not exact match. If agents still push to main, check that the prompt augmentation is intact.
- **Dependency resolution trigger**: `resolveDependents()` only fires through `onTaskTerminal()` in the agent manager and `sprint:update` IPC handler. Direct SQLite writes that bypass the IPC handlers do NOT trigger dependent unblocking. Use the IPC handler or manually queue blocked tasks.
...
- **Sprint partition buckets**: UI components must use the 7 buckets from `partitionSprintTasks()` (`backlog`, `todo`, `blocked`, `inProgress`, `awaitingReview`, `done`, `failed`) — NOT raw task statuses. `awaitingReview` = active/done with `pr_status=open`. `failed` = failed + error + cancelled.
- **Neon styling convention**: All views use CSS classes in dedicated `*-neon.css` files with `var(--neon-*)` custom properties (defined in `neon.css`). Neon CSS files: `agents-neon.css`, `sprint-pipeline-neon.css`, `task-workbench-neon.css`, `sprint-neon.css` (legacy). Do NOT use inline `tokens.*` styles for neon views — use CSS classes. BEM-like naming per view: `.console-*` (agents), `.pipeline-*`/`.task-pill` (pipeline), `.wb-*` (workbench).
