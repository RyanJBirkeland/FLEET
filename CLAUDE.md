...
- **Views**: 10 views in `src/renderer/src/views/` — Dashboard (⌘1), Agents (⌘2), IDE (⌘3, default), Sprint (⌘4), PR Station (⌘5), Source Control (⌘6), Memory (⌘7), Cost (⌘8), Settings (⌘9), Task Workbench. View type union and `VIEW_LABELS` live in `panelLayout.ts`. Keyboard shortcuts mapped in `App.tsx` via `VIEW_SHORTCUT_MAP`.
- **IDE**: `src/renderer/src/views/IDEView.tsx` + `src/renderer/src/components/ide/` (10 components). Monaco editor + file explorer sidebar + integrated terminal. `ideStore` in `src/renderer/src/stores/ide.ts`. File I/O via `ide-fs-handlers.ts` (path-scoped to opened root, atomic writes, binary detection). State persisted to `ide.state` setting with 2s debounce.
...
- **Worktree /tmp path issue (macOS)**: `/tmp` is a symlink to `/private/tmp` on macOS. Git worktrees created under `/tmp/worktrees/` can lose tracked files between operations because tools resolve the path inconsistently. Fix with `git checkout -- .` to restore. `DEFAULT_CONFIG.worktreeBase` now defaults to `~/worktrees/bde` (changed in PR #419). Users with old `/tmp` settings should update via Settings UI.
- **Testing worktree changes from main repo**: Worktree `npm run test:main` often fails due to `better-sqlite3` rebuild issues. Workaround: `git cherry-pick --no-commit <hash>` into the main repo, run tests there, then `git checkout -- .` to restore. This is faster and more reliable than fixing native modules in worktrees.
...
- **Agents default to main branch**: Fixed in PR #418 — `run-agent.ts` now auto-appends `## Git Branch` with explicit branch name. If agents still push to main, check that the prompt augmentation in `run-agent.ts` is intact.
- **Agent prompt augmentation**: `run-agent.ts` auto-appends `## Setup` (npm install reminder) and `## Git Branch` (explicit branch name + "do not push to main") to every agent prompt. When adding new prompt sections, append after these. When testing prompt content in tests, use `expect.stringContaining()` not exact match.
- **Dependency resolution trigger**: `resolveDependents()` only fires through `onTaskTerminal()` in the agent manager and `sprint:update` IPC handler. Direct Supabase PATCH to `status: 'done'` does NOT trigger dependent unblocking. Use the IPC handler or manually queue blocked tasks.
- **Coverage artifacts not gitignored**: The `coverage/` directory is generated by `npm run test:coverage` but is NOT in `.gitignore`. Agents may accidentally commit it. If this happens, `git rm -r --cached coverage/` and add to `.gitignore`.
