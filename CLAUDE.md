...
- **Pushing from worktrees (fallback)**: If `cd` into a worktree fails (macOS path loss), push the branch from the main repo instead: `git push origin <branch-name> --no-verify`. The commit is in the branch regardless of where you push from.
...
- **sprint_tasks full column list (v22)**: id, title, prompt, repo, status, priority, depends_on, spec, notes, pr_url, pr_number, pr_status, pr_mergeable_state, agent_run_id, retry_count, fast_fail_count, started_at, completed_at, claimed_by, template_name, playground_enabled, needs_review, max_runtime_ms, spec_type, worktree_path, session_id, created_at, updated_at. Verify against `db.ts` migrations before writing new ones.
...
- **Neon styling convention**: All views use CSS classes in dedicated `*-neon.css` files that layer on top of base CSS (don't modify base files). Import neon file AFTER base in `main.css`. Neon CSS files: `agents-neon.css`, `sprint-pipeline-neon.css`, `task-workbench-neon.css`, `ide-neon.css`, `code-review-neon.css`, `diff-neon.css`, `dashboard-neon.css`, `sprint-neon.css` (legacy). Do NOT use inline `tokens.*` styles for neon views — use CSS classes. BEM-like naming per view: `.console-*` (agents), `.pipeline-*`/`.task-pill` (pipeline), `.wb-*` (workbench), `.review-*` (code review), `.dashboard-*` (dashboard). Shared `.bde-btn` classes need scoped overrides inside neon views (e.g., `.review-station .bde-btn--ghost`). Remaining views without neon: Source Control (`.git-*` classes in `diff.css`), Settings.
...
- **Post-merge test sync**: After merging multiple agent PRs, run `npm run test:main` immediately. Agent PRs frequently update production code without updating tests (string assertions, mock shapes, new exports). Fix tests to match production — never change production to match stale tests.
...
- **SDK query() is single-turn**: `sdk.query()` iterator ends when model emits a `result` message. For multi-turn adhoc agents, use `query()` per turn with `resume: sessionId` to continue the conversation. The session ID comes from the `system/init` message. `maxTurns`, `streamInput()`, and v2 `session.stream()` all terminate per-turn — they don't keep the session alive.
- **Clear `out/` after main process changes**: Vite HMR only hot-reloads the renderer. Main process changes (adhoc-agent, tearoff-manager, handlers) require `rm -rf out/` and restart `npm run dev` to take effect. The `predev` script rebuilds better-sqlite3 but doesn't clear the build cache. This also applies after `git pull` — always `rm -rf out/` before restarting if merged PRs touched main process code.
- **Pipeline agents must use `canUseTool`**: Pipeline agents are non-interactive (no human at stdin). Without `canUseTool: async () => ({ behavior: 'allow' })`, any unapproved tool hangs forever waiting for stdin permission. Safety comes from worktree isolation + PR review, not permission prompts.
- **Agent permissions in `~/.claude/settings.json`**: Agent tool permissions (allow/deny) live in the Claude Code CLI config file, NOT in BDE's SQLite settings table. Read/write via `claude:getConfig`/`claude:setPermissions` IPC (registered in `claude-config-handlers.ts`). `claude-settings-bootstrap.ts` applies BDE defaults on first launch if no permissions configured.
