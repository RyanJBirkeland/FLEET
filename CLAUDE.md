...
- PR Builder backend: `src/main/services/pr-group-build-service.ts` — `PrGroupBuildService.buildGroup(id)` handles single-task push and multi-task squash-merge rollup; `checkConflicts(id)` dry-runs before committing. `src/main/data/pr-group-queries.ts` — CRUD for `pr_groups` table. `src/main/handlers/pr-groups.ts` — IPC thin wrapper.
- PR Builder frontend: `src/renderer/src/components/code-review/PrBuilderModal.tsx` — full-screen drag-and-drop PR group composer. `src/renderer/src/stores/prGroups.ts` — Zustand store. `src/renderer/src/hooks/usePrGroups.ts` — group management actions scoped by repo. `src/renderer/src/hooks/useApproveAction.ts` — transitions a task `review → approved` with toast feedback.
- Task stacking: `stacked_on_task_id` on `sprint_tasks` records which approved upstream task a downstream agent's worktree was forked from. Set by the drain loop in `src/main/agent-manager/task-claimer.ts`. The success pipeline in `src/main/agent-manager/resolve-success-phases.ts` auto-rebases stacked branches onto `origin/main` before promoting to `review`.
...
- **Feature flags**: `src/renderer/src/stores/featureFlags.ts` — localStorage-backed (`fleet:ff`). All five flags now default `true` — V2 is the production UI. Flags exist only for emergency rollback (set a flag to `false` in DevTools to force V1). **Next step: delete V1 files, dispatcher shims, and this store entirely.** V1 tests that still exist mock `v2*: false` explicitly to preserve coverage until V1 is deleted.
- **Shell components**: `UnifiedHeader.tsx` and `Sidebar.tsx` are thin dispatcher shims (pending deletion with V1 cleanup). Active implementations: `UnifiedHeaderV2.tsx` (38px, F mark + FLEET wordmark, CommandPill + HealthChip + TokenChip), `SidebarV2.tsx` (200px, labeled nav with accent rail, live agents block, collapses to 52px at <1024px via container query). V2-only primitives in `layout/`: `CommandPill`, `HealthChip`, `TokenChip`, `LiveAgentRow`. V1 files (`UnifiedHeaderV1`, `SidebarV1`, css files) are dead code pending deletion.
- **Neon components**: `src/renderer/src/components/neon/` (11 primitives: NeonCard, StatCounter, NeonBadge, GlassPanel, ActivityFeed, NeonProgress, PipelineFlow, SankeyPipeline, MiniChart, StatusBar, NeonTooltip). V1-era glass/glow aesthetic — **do not introduce new usages.** Pending deletion as part of V1 cleanup.
...
- **Code Review**: `src/renderer/src/views/CodeReviewView.tsx` + `src/renderer/src/components/code-review/` (ReviewQueue, ReviewDetail, ReviewActions, ChangesTab, CommitsTab, ConversationTab). `codeReview` Zustand store. Agent completion stops at `review` status with worktree preserved. Sidebar splits into "Pending Review" (`review`) and "Approved" (`approved`) sections. **Approve** transitions `review → approved` and immediately unblocks hard-dep downstream tasks. Approved tasks can be grouped into stacked PRs via the **PR Builder** (`PrBuilderModal.tsx` + `PrGroupBuildService`) — single-task groups push the existing branch; multi-task groups squash-merge in dependency order onto a rollup branch. Sprint PR Poller drives `approved → done` when the GitHub PR merges.
...
- **Task Pipeline**: `src/renderer/src/components/sprint/SprintPipeline.tsx` — three-zone layout (PipelineBacklog | PipelineStage×6 | TaskDetailDrawer). Uses `partitionSprintTasks()` for stage mapping (9 buckets: backlog, todo, blocked, inProgress, pendingReview, **approved**, openPrs, done, failed). Neon CSS in `sprint-pipeline-neon.css`. Task creation lives in the `TaskWorkbenchModal`, opened via `useTaskWorkbenchModalStore.openForCreate()`; clicking Edit on a task calls `openForEdit(task)` (no view jump).
