...
- **Task creation must use IPC**: Always create sprint tasks via `sprint:create` IPC handler (or Sprint Center UI), not direct Supabase REST. Direct writes skip the auto-blocking logic in `sprint-local.ts`, and the drain loop doesn't check `depends_on` before claiming tasks.
...
- **Neon styling convention**: Sprint and Agents views use CSS classes in dedicated `*-neon.css` files with `var(--neon-*)` custom properties (defined in `neon.css`). Do NOT use inline `tokens.*` styles for neon views — use CSS classes. The `neonVar()` helper in `components/neon/types.ts` maps accent names to CSS vars.
