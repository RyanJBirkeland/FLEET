...
- **IDE**: `src/renderer/src/views/IDEView.tsx` + `src/renderer/src/components/ide/` — Monaco editor + file explorer sidebar + integrated terminal. Uses `ideStore` for state, `@monaco-editor/react` for editor. State persisted to `ide.state` setting (debounced 2s) and restored on mount.
...
- **Integration tests**: `src/main/__tests__/integration/` — covers AgentManager pipeline, AuthGuard, IPC handlers, Queue API, and CompletionHandler. Run with `npm run test:main`. All tests passing (~637 main, ~1571 renderer, 36 E2E). Coverage threshold enforced at 68%.
