...
interface DesignModeContentProps {
  repo: string
  priority: number
  onSave: (args: { title: string; spec: string; repo: string; description: string; priority: number }) => void
}
...
function extractTitle(text: string): string | null {
  const match = text.match(
    /(?:Ticket Title|Title|Suggested title):\s*["']?([^\n"']+)["']?/i
  )
  return match?.[1]?.trim() ?? null
}
...
export function DesignModeContent({
  ...
  const sendMessage = useCallback(async () => {
    ...
    const newUserMessage: DesignMessage = {
      role: 'user',
      content: userText,
      timestamp: Date.now(),
    }
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: 'assistant',
          content:
            "Sorry, I couldn't reach the gateway. Check your connection and try again.",
          timestamp: Date.now(),
        },
      ])
    } finally {
            ...
            <>
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: renderMarkdown(specDraft),
                }}
              />
