...
describe('SprintTaskList', () => {
  ...
  it('filters tasks by status', () => {
    const onSelectTask = vi.fn()
    const { container } = render(<SprintTaskList tasks={mockTasks} selectedTaskId={null} onSelectTask={onSelectTask} />)

    const filtersSection = container.querySelector('.sprint-task-list__filters') as HTMLElement
    const backlogFilter = within(filtersSection).getByRole('button', { name: /Backlog/ })
  ...
  it('filters tasks by done status', () => {
    ...
    const filtersSection = container.querySelector('.sprint-task-list__filters') as HTMLElement
    const doneFilter = within(filtersSection).getByRole('button', { name: /Done/ })
  ...
  it('disables filter chips with zero count', () => {
    ...
    const { container } = render(<SprintTaskList tasks={tasksWithNoFailed} selectedTaskId={null} onSelectTask={onSelectTask} />)

    const filtersSection = container.querySelector('.sprint-task-list__filters') as HTMLElement
    const failedFilter = within(filtersSection).getByRole('button', { name: /Failed/ })
