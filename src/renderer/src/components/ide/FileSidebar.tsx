...
export function FileSidebar({ onOpenFile }: FileSidebarProps): React.JSX.Element {
  ...
  function handleCopyPath(path: string): void { void navigator.clipboard.writeText(path) }
    ...
    <div className="ide-sidebar" onContextMenu={(e) => e.preventDefault()}>
      <div className="ide-sidebar__header">
        ...
        <div className="ide-sidebar__actions">
          <button className="ide-sidebar__action-btn" onClick={() => void handleOpenFolder()} title="Open Folder" aria-label="Open Folder">
            <FolderOpen size={14} />
          </button>
          <button className="ide-sidebar__action-btn" onClick={toggleSidebar} title="Close Sidebar" aria-label="Close Sidebar">
      ...
      <div className="ide-sidebar__body">
        {rootPath
          ...
          : <div style={{ padding: '12px 8px', fontSize: 'var(--bde-size-sm)', color: 'var(--bde-text-dim)' }}>No folder open</div>
        }
      </div>
      {contextMenu && (
        <FileContextMenu target={contextMenu}
