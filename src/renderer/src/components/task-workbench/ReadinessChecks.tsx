  ...
  return (
    <div style={{
      ...
      <button
        ...
        style={{
          background: 'none', border: 'none', color: tokens.color.text,
          cursor: 'pointer', display: 'flex', alignItems: 'center',
          gap: tokens.space[2], width: '100%', fontSize: tokens.size.sm, padding: 0,
        }}
      >
        <span>{expanded ? '\u25be' : '\u25b8'}</span>
        <span style={{ display: 'flex', gap: tokens.space[1] }}>
          {allChecks.map((c) => <span key={c.id} title={c.label}>{STATUS_ICONS[c.status]}</span>)}
        </span>
        <span style={{ color: tokens.color.textMuted, marginLeft: 'auto' }}>{passing}/{total} passing</span>
      </button>
      {expanded && (
